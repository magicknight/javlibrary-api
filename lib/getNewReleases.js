...
import { URL_NEWRELEASES, parseList } from '../utils/helper';
 ...
 * */
const getNewReleases = async({ order = 1, page = 1 } = {}) => {
    try {
        var request = createRequest();
        var response = await request.get(`${URL_NEWRELEASES}?page=${page}&mode=${order}`);
