...
import { URL_LISTBYTAG, parseList } from '../utils/helper';
 ...
 * @param {number} options.id - The tag id.
 * @param {number} options.page - The page index.
 * @returns {object|undefined} A object the if successful. If failure not returned.
 * */
const listByTag = async({ id, page = 1 } = {}) => {
    try {
        var request = createRequest();
        var response = await request.get(`${URL_LISTBYTAG}${id}&page=${page}`);
