...
import { URL_LISTBYMAKER, parseList } from '../utils/helper';
 ...
 * @param {number} options.id - The maker id.
 * @param {number} options.page - The page index.
 * @returns {object|undefined} A object the if successful. If failure not returned.
 * */
const listByMaker = async({ id, page = 1 } = {}) => {
    try {
        var request = createRequest();
        var response = await request.get(`${URL_LISTBYMAKER}${id}&page=${page}`);
